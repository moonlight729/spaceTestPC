...
int main(void)
{
    ...
    if (camera_stream_run_test(&request, &result) == 0) {
        printf("PASS device=%s captured=%d exposureDelta=%d streamOk=%d exposureOk=%d message=%s\n",
               result.device_path, result.captured_frames, result.exposure_delta,
               result.stream_ok ? 1 : 0, result.exposure_ok ? 1 : 0, result.message);
        return 0;
    }

    printf("FAIL code=%d device=%s captured=%d exposureDelta=%d message=%s\n",
           result.error_code, result.device_path, result.captured_frames,
           result.exposure_delta, result.message);
    return 1;
}
