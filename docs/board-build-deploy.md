# 板端设备登录、源码检查、编译与服务部署指南

> 本文以当前联调设备 `192.168.110.76`（下文简称 `110.76`）为例，替换 IP 后适用于其它同型号工位。
> 相关文档：[`communication-protocol.md`](communication-protocol.md)（协议）、
> [`application-upgrade.md`](application-upgrade.md)（上位机自动升级）、
> [`board-deployment-hardening.md`](board-deployment-hardening.md)（板端自愈与固化）。
>
> 提示：上位机 SSH/SCP 升级通道使用的账号口令来自 `appsettings.json` 的 `upgrade.sshUser` /
> `upgrade.sshPassword`，请勿把真实口令提交到 Git；本文统一用占位符 `设备密码` 表示。
