...
> **状态：部分落地（截至 2026-09）。** 上位机侧已经把 USB 预检拆成 `usb2` / `usb3` 两个正式测试项，
> 并在界面上展示插入/拔出分步步骤（`Usb2TestSteps` / `Usb3TestSteps`），参数见 `appsettings.json`
> 的 `testParameters.usb2` / `usb3`；板端是否完全按本文的单服务预检 + 结果文件方式实现仍需联调确认。
> 上位机现状见 [`../architecture.md`](../architecture.md)，协议口径见 [`../communication-protocol.md`](../communication-protocol.md) 第 16.5 节。
