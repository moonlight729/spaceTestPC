...
> **状态：已实现（截至 2026-09）。**
> 上位机实现位置：`SpaceTestPC.App/Services/AdbPcbaCommandClient.cs`（传输与远端脚本）
> 与 `SpaceTestPC.App/ViewModels/MainViewModel.cs` 的 `EnsureApplicationUpgradeCoreAsync`（决策与交互）。
> 配置来源：`appsettings.json` 的 `upgrade` 段（见 [`configuration.md`](configuration.md)）。
> 板端自愈与回滚见 [`board-deployment-hardening.md`](board-deployment-hardening.md)。

与本文档写稿时的差异（以下以代码为准）：

- 自动升级倒计时不再是固定的 5 秒，改为 `upgrade.autoUpgradeDelaySeconds`（当前配置 1 秒，最小取 1）。
- 传输方式由连接方式决定：`upgrade.transport` 为 `auto` 时，`tcp` 模式走 **SSH/SCP**，`adbForward` 模式走 **ADB 脚本**；也可显式指定 `adb` / `sshScp`。
- 升级检查由后台定时器驱动（首次扫码前就开始），并在序列面板以 `application_upgrade` 伪测试项展示；`mock` 模式或 `upgrade.enabled=false` 时不执行任何升级动作。
- 版本号不作为升级判据（代码里已显式停用）：**仅 MD5 决定是否需要升级**。
